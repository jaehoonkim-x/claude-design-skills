...
description: "[Multi-Level] Figma URL 또는 Pencil(.pen) 파일의 선택된 프레임에 대해 design-{ui,ux,ceo}-*-review 23개 스킬을 5×N wave 로 분할 dispatch (rate limit 회피) + Pencil/Figma snapshot 1회 캐싱 + conditional 사전 skip 으로 L0 Surface(UI 10) + L1 Skeleton(UX 6) + L2 Structure(UX 5) + L5 Strategy(CEO+JTBD 2) 한국어 리뷰 보고서 + 통합 집계 출력. 사용자가 \"전체 디자인 리뷰\", \"all design review\", \"풀 리뷰\", \"design audit all\", \"design-review-all\", \"/design-review-all\" 를 말하거나 figma.com/design URL 또는 .pen 파일로 다층(L0-L5) 디자인 검토를 한 번에 요청할 때 사용."
---
...
**목적**: 23 서브에이전트가 동일 Pencil/Figma 데이터를 23회 중복 fetch 하는 것을 방지 + 적용 불가 스킬을 dispatch 단계에서 제거.
...
- 집계 보고서 **1개**: `./design-reviews/design-review-all-{frame-slug}-{YYYYMMDD-HHmm}.md`
  - 레벨별 요약 테이블 (L0 10 + L1 6 + L2 5 + L5 2 = 23개) + 통합 finding (dedupe + source tag) + 우선순위 + 원본 매핑 한 파일
...
| 3 | Pencil/Figma snapshot 1회 캐싱 (Step 2.5A·B) | 23회 중복 fetch 제거 → 토큰 ~25% 감소 |
| 4 | Schema 1회 inject (`include_schema: false`) | 23 × ~8k 토큰 중복 제거 |
...
- wall-clock: ~12-15분 (병렬 23) + retry 사이클 ~5분 → 실측 ~20분
...
권장 워크플로:
```
/design-review-all <design 파일>        → 23 + 1개 .md 생성
...
| L2 Structure | UX Flow/Eval | 5 | frame 시퀀스 / hub | flow·IA·dark·CW·PURE·HEART |
